...
const WalletButton = dynamic(
  () => import("./wallet/WalletButton").then((mod) => mod.WalletButtonInner),
  {
    ssr: false,
    loading: () => (
      <button className="bg-gold/10 border border-gold/30 text-gold px-4 py-2 text-[13px] opacity-50">
        Connecting...
      </button>
    ),
  }
...
export const Navigation = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-obsidian/98 backdrop-blur-md border-b border-slate-light/30">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between py-4">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display text-lg text-ivory hidden sm:block">
              TribunalCraft
            </span>
          </Link>
      ...
      {/* Mobile Navigation */}
      <div className="md:hidden border-t border-slate-light/30">
